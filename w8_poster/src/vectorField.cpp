...
vectorField::vectorField(){
    ...
    for (int i = 0; i < 3; i++) {
        ofColor c;
        snowflakeColors.push_back(c);
    }
    
    snowflakeColors[0].set(32, 57, 84);
    snowflakeColors[1].set(112, 153, 75);
    snowflakeColors[2].set(43, 16, 59);
...
void vectorField::draw(){
    ...
    for (int i = 0; i < fieldWidth; i++){
        for (int j = 0; j < fieldHeight; j++){
            ...
            ofSetColor(255, 292, 191, 100);
            if (length < 300)                    ofSetColor(snowflakeColors[0], 100);
            if (length > 300 && length < 600)    ofSetColor(snowflakeColors[1], 100);
            if (length >= 600)                  ofSetColor(snowflakeColors[2], 100);
            
			//ofLine(px - line.x*length*0.2, py - line.y*length*0.2, px + line.x*length*0.2, py + line.y*length*0.2);
			ofBeginShape();
            ofCurveVertex(px-line.x*length*0.2, py-line.y*length*0.2);
            ofCurveVertex(px+line.x*length*0.2, py+line.y*length*0.2);
            ofEndShape();
