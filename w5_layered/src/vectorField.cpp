...
void vectorField::randomizeField(float scale){
	for (int i = 0; i < fieldSize; i++){
        ...
        float x = ofMap(i, 0, fieldSize, -cos(ofGetElapsedTimef()), cos(ofGetElapsedTimef()) ) * scale;
        float y = ofMap(i, 0, fieldSize, sin(ofGetElapsedTimef()), -sin(ofGetElapsedTimef()) ) * scale;
